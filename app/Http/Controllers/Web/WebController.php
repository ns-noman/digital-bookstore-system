...
class WebController extends Controller
{
    ...
    public function home()
    {
        ...
        $latest_products = Product::with(['reviews'])->active()->orderBy('id', 'desc')->take(24)->get();
        $categories = Category::where('position', 0)->take(12)->get();
        $topcategories = Category::where('position', 0)->take(12)->get();
        ...
        return view('web-views.home', compact('featured_products', 'topRated', 'bestSellProduct', 'latest_products', 'categories', 'brands', 'deal_of_the_day', 'top_sellers', 'home_categories','topcategories'));
    }
    ...
    public function checkout_details(Request $request)
    {
        ...
        if (count($cart_group_ids) > 0) {
            ...
            return view('web-views.checkout-shipping',$data);
    ...
    public function products(Request $request)
    {
        ...
        if ($request['data_from'] == 'search') {
            ...
            $query = $porduct_data->where(function ($q) use ($key) {
                foreach ($key as $value) {
                    $q->orWhere('name', 'like', "%{$value}%");
                    $q->orWhere('product_uid', 'like', "%{$value}%");
                }
            });
        }
    ...
    public function nillgirimall(Request $request)
    {
        $request['sort_by'] == null ? $request['sort_by'] == 'latest' : $request['sort_by'];
        ...
        $query = Product::with(['reviews'])->active()->where('mall_status', 1);
        ...
        $products = $fetched->paginate(54)->appends($data);
        ...
        return view('web-views.mall.view', compact('products', 'data'), $data);
    }
